...
const (
	inputFileName = "linkedmap.in"
	outputFileName = "linkedmap.out"
)
...
func (lhm *LinkedHashMap) Next(key interface{}) (nextKey, nextValue interface{}) {
	var node = lhm.find(key)
	if node != nil && node.nextAdded != nil {
		nextKey, nextValue = node.nextAdded.key, node.nextAdded.value
	}
...
func main() {
	...
	for nextLinePresent := readNext(&currentCommand, &key, &value); nextLinePresent; nextLinePresent = readNext(&currentCommand, &key, &value) {
		...
		case "get":
			if value := set.Get(key); value != nil {
				printf("%s\n", value.(string))
			} else {
				printf("none\n")
			}
		case "delete":
			set.Delete(key)
		case "prev":
			if _, prevValue := set.Previous(key); prevValue != nil {
				printf("%s\n", prevValue.(string))
			} else {
				printf("none\n")
			}
		case "next":
			if _, nextValue := set.Next(key); nextValue != nil {
				printf("%s\n", nextValue.(string))
			} else {
				printf("none\n")
			}
