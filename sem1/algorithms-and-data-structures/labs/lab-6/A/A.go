...
const (
	inputFileName = "set.in"
	outputFileName = "set.out"
)
...
func readNext(nextCommand *string, nextValue *int) bool {
	_, err := scanf("%s %d\n", nextCommand, nextValue)
	return err == nil
}
...
func main() {
	...
	var (
		...
		value int
		set = MakeLinkedHashMap(2, func(i interface{}) uint64 {
			switch i.(type) {
			case int:
				return uint64(i.(int))
			default:
	...
	for nextLinePresent := readNext(&currentCommand, &value); nextLinePresent; nextLinePresent = readNext(&currentCommand, &value) {
		switch currentCommand {
		case "insert":
			set.Insert(value, true)
		case "exists":
			if set.Get(value) != nil {
				printf("true\n")
			} else {
				printf("false\n")
			}
