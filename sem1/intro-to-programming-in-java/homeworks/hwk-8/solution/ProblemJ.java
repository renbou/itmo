...
//    }
    ...
    public static void main(String[] args) {
        ...
        byte[][] pathCountMatrix = new byte[n][n];

        for (int i = 0; i < n; i++) {
            String currentLine = stdinScanner.next();
            for (int j = 0; j < n; j++) {
                pathCountMatrix[i][j] = (byte)(currentLine.charAt(j) - '0');
            }
        }

        for (int i = 0; i < n; i++) {
            char[] currentAdjacencyList = new char[n];
            Arrays.fill(currentAdjacencyList, '0');
            for (int j = i + 1; j < n; j++) {
                if (pathCountMatrix[i][j] != 0) {
                    currentAdjacencyList[j] = '1';
                    for (int k = j + 1; k < n; k++) {
                        pathCountMatrix[i][k] = (byte)((pathCountMatrix[i][k] - pathCountMatrix[j][k]) % 10);
                    }
                }
            }
