 ...
 * traversal of which can be done from both incident vertices
 */
...
bool UndirectedEdge<G>::connects(const IVertex &u, const IVertex &v) const {
  return (this->first == u and this->second == v) or
         (this->first == v and this->second == u);
}

/*
 * Edge class representing a directed edge,
 * traversal of which can be done from the first vertice to the second
 */
template <class G> class DirectedEdge : public BaseEdge<G> {
public:
  static DirectedEdge<G> Invalid;
...
template <class G> DirectedEdge<G> DirectedEdge<G>::Invalid{};

template <class G>
const typename G::Vertex &
DirectedEdge<G>::traverseFrom(const IVertex &v) const {
  if (v == this->first) {
    return this->second;
  }
  return G::Vertex::Invalid;
...
template <class G>
bool DirectedEdge<G>::connects(const IVertex &u, const IVertex &v) const {
  return this->first == u and this->second == v;
}
...
AdjacencyListGraph<V, E>::addEdge(const IdT &u, const IdT &v) {
  if (this->template validId(_vertices, u) and
...
using Graph = AdjacencyListGraph<BaseVertex, DirectedEdge>;

using intVec = std::vector<int>;

std::pair<std::vector<int>, int> markSCC(Graph &g) {
  std::vector<bool> visited(g.numVertices(), false);
  std::vector<int> ordered;
  ordered.reserve(g.numVertices());

  std::function<void(const Graph::Vertex &v)> internal;
  internal = [&](const Graph::Vertex &v) {
    visited[v] = true;
  ...
  // First traversal to mark order
  for (const Graph::Vertex &v : g.vertices()) {
    if (not visited[v]) {
      internal(v);
    }
  }
  ...
  // Transpose the graph
  Graph transposed(g.numVertices(), g.numEdges());
  for (const Graph::Vertex &v : g.vertices()) {
    for (const Graph::Edge &e : v.outgoingEdges()) {
      transposed.addEdge(e.traverseFrom(v), v);
    }
  }

  std::vector<int> scc(g.numVertices(), -1);
  int component = -1;

  std::function<void(const Graph::Vertex &v)> mark;
  mark = [&](const Graph::Vertex &v) {
    visited[v] = true;
    scc[v] = component;
    for (const Graph::Edge &e : v.outgoingEdges()) {
      const auto &to = e.traverseFrom(v);
  ...
  visited.assign(transposed.numVertices(), false);
  for (int v : ordered) {
    if (not visited[v]) {
      component++;
      mark(transposed.vertex(v));
    }
  }

  return {scc, component + 1};
}

int countSCCEdges(Graph &g) {
  auto [markedSCC, components] = markSCC(g);
  std::vector<std::unordered_set<int>> scc(components);
  for (const Graph::Vertex &v : g.vertices()) {
    for (const Graph::Edge &e : v.outgoingEdges()) {
      const auto &to = e.traverseFrom(v);
      if (markedSCC[v] != markedSCC[to]) {
        scc[markedSCC[v]].emplace(markedSCC[to]);
      }
    }
  }

  int numEdges = 0;
  for (const auto &edges : scc) {
...
int main() {
  ...
  std::cout << countSCCEdges(g) << std::endl;
  return 0;
}