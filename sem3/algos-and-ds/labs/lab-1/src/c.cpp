...
std::vector<Graph::ConstVertexRef> findArticulationPoints(Graph &g) {
  intVec tin(g.numVertices(), -1);
  intVec low(g.numVertices(), -1);
  ...
  internal = [&](const Graph::Vertex &v, const Graph::Vertex &p) {
    ...
    for (const Graph::Edge &e : v.outgoingEdges()) {
      ...
      if (tin[to] == -1) {
        ...
        internal(to, v);
        low[v] = std::min(low[v], low[to]);
        if (low[to] >= tin[v] && p != Graph::Vertex::Invalid) {
    ...
    if (children > 1 && p == Graph::Vertex::Invalid) {
      articulationPoints.push_back(v);
    }
  };
