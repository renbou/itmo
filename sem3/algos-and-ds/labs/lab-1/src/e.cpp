...
std::pair<std::vector<int>, int> markVerticeBiconnected(Graph &g) {
  intVec tin(g.numVertices(), -1);
  intVec low(g.numVertices(), -1);
  std::vector<Graph::ConstVertexRef> articulationPoints;
  int time = 0;

  std::function<void(const Graph::Vertex &v, const Graph::Vertex &p)> internal;
  internal = [&](const Graph::Vertex &v, const Graph::Vertex &p) {
    tin[v] = low[v] = time++;
    int children = 0;
  ...
  for (Graph::Vertex &v : g.vertices()) {
    if (tin[v] == -1) {
      internal(v, Graph::Vertex::Invalid);
    }
  }

  int component = 0;
  std::vector<int> marked(g.numEdges(), -1);
  std::vector<bool> visited(g.numVertices(), false);
  std::function<void(const Graph::Vertex &, const Graph::Vertex &, int)> paint;
  paint = [&](const Graph::Vertex &v, const Graph::Vertex &p,
              int currentComponent) {
    visited[v] = true;
    for (const Graph::Edge &e : v.outgoingEdges()) {
      const Graph::Vertex &to = e.traverseFrom(v);
      if (to == p) {
        // Mark all parallel edges
        marked[e] = currentComponent;
        continue;
      }

      if (not visited[to]) {
        // Haven't been, so either new component or same one
        if (low[to] >= tin[v]) {
          // New component
          component++;
          marked[e] = component;
          paint(to, v, component);
        } else {
          marked[e] = currentComponent;
          paint(to, v, currentComponent);
        }
      } else if (tin[to] < tin[v]) {
        // Already visited same component
        marked[e] = currentComponent;
      }
    }
  };
  ...
  for (Graph::Vertex &v : g.vertices()) {
    if (visited[v] == false) {
      paint(v, Graph::Vertex::Invalid, component);
    }
  }
...
int main() {
  ...
  const auto [marked, component] = markVerticeBiconnected(g);
