...
const std::pair<int, int> InvalidPair = {-1, -1};

void dfsLinks(Graph &linkGraph, std::vector<std::pair<int, int>> &occurrences,
              int v) {
  for (const int to : linkGraph[v]) {
    dfsLinks(linkGraph, occurrences, to);
    if (occurrences[to] != InvalidPair) {
      if (occurrences[v] == InvalidPair) {
        occurrences[v] = occurrences[to];
      } else {
        occurrences[v] = {
            std::min(occurrences[v].first, occurrences[to].first),
            std::max(occurrences[v].second, occurrences[to].second)};
      }
    }
  }
}
...
int main() {
  ...
  // and keep first and last occurence of vertex
  std::vector<std::pair<int, int>> occurrences(ahoCorasick.size(), InvalidPair);
  int v = 0;
  for (const int i : Range(text.size())) {
    v = ahoCorasick.go(v, text[i]);
    if (occurrences[v] == InvalidPair) {
      // first encounter, set it only once
      occurrences[v].first = i;
    }
    // and always update the last encounter
    occurrences[v].second = i;
  }

  // now build reversed link graph (v->u if u's link is v)
  // so that we know which vertices' occurrences are to be taken into account
  // for v's first and last occurrences, since if they are found, then their
  // suffix==link is also found -> occurrence must be updated for suffix
  ...
  dfsLinks(linkGraph, occurrences, 0);

  // add matched ids to set
  std::vector<std::pair<int, int>> stringOccurrences(n, InvalidPair);
  for (const int i : Range(ahoCorasick.size())) {
    const auto &vertex = ahoCorasick.getVertex(i);
    if (occurrences[i] != InvalidPair && vertex.terminal) {
      for (const int matchIndex : vertex.stringIndices) {
        stringOccurrences[matchIndex] = occurrences[i];
      }
    }
  }

  for (const int i : Range(n)) {
    if (stringOccurrences[i] == InvalidPair) {
      io.write("-1 -1");
    } else {
      io.write(stringOccurrences[i].first - strings[i].length() + 1);
      io.sp();
      io.write(stringOccurrences[i].second - strings[i].length() + 1);
    }
    io.nl();
  }
}