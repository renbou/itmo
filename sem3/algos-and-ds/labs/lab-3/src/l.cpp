...
public:
  ...
  IntVec sortCyclicShifts(const std::string &str) {
    ...
    this->numEquivalenceClasses = equivalenceClass + 1;
    this->equivalenceClasses.swap(equivalenceClasses);
    return suffixArray;
  }
  ...
  IntVec getSortedCyclicShifts() {
    const int n = equivalenceClasses.size();
    IntVec sortedCyclicShifts(numEquivalenceClasses, 0);
    for (const int i : Range(n)) {
      sortedCyclicShifts[equivalenceClasses[i]] = i;
    }
    return sortedCyclicShifts;
  }

  // equivalence classes of the cyclic substrings of the string
  int numEquivalenceClasses;
  IntVec equivalenceClasses;
  IntVec suffixArray;
  IntVec lcp;
};
...
int main() {
  ...
  const int k = io.read<int>();
  SuffixArray suffixArray(s);
  const auto sortedCyclicShifts = suffixArray.getSortedCyclicShifts();

  if (k > sortedCyclicShifts.size()) {
    io.write("IMPOSSIBLE");
  } else {
    io.write(s.substr(sortedCyclicShifts[k - 1]));
    io.write(s.substr(0, sortedCyclicShifts[k - 1]));
  }
  io.nl();
}