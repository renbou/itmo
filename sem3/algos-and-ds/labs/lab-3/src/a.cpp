...
public:
  ...
  void update(const char c) {
    hash = int64(
        (__int128(hash) + __int128(c - 'a' + 1) * getPowerOfP(powerIndex)) % m);
    powerIndex++;
  }
...
int main() {
  ...
  for (const auto _ : Range(q)) {
    const auto a = readInt() - 1;
    const auto b = readInt() - 1;
    const auto c = readInt() - 1;
    const auto d = readInt() - 1;

    if (sHashed.substringHash(a, b) == sHashed.substringHash(c, d)) {
      writeString("Yes");
    } else {
      writeString("No");
    }
