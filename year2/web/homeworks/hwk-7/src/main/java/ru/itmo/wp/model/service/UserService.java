...
public class UserService {
    ...
    public void validateAdminPermissions(User user) throws ValidationException {
        if (user == null) {
            throw new ValidationException("Must be logged in as admin");
        }
        user = this.find(user.getId());
        if (!user.getAdmin()) {
            throw new ValidationException("Admin role required for action");
        }
    }

    public void validateAndToggleAdmin(User admin, User user) throws ValidationException {
        this.validateAdminPermissions(admin);
        ...
        userRepository.toggleAdmin(user);
    }
    ...
    public long validateUserId(String userId) throws ValidationException {
        try {
            ...
            throw new ValidationException("userId must represent a valid number");
        }
    }
